...
class HomTex:
    ...
    def Test(self, model, epoch, output_dir):
        ...
        cain_dir = '/mnt/storage/home/mt20523/CAIN'
        model.eval()
        ...
        for i, yuv_path in enumerate(self.yuv_list, 1):
            ...
            # # compute vmaf
            # os.chdir(vmaf_dir)
            # cmd = 'PYTHONPATH=python /content/vmaf/python/vmaf/script/run_vmaf.py yuv420p 256 256 {} {}/tmp.yuv --out-fmt text'.format(yuv_path, cain_dir)
        ...
        with open(output_dir+'/cain_psnr_epoch{}.pkl'.format(str(epoch).zfill(2)), 'wb+') as f:
            pickle.dump(psnr_dict, f)
        with open(output_dir+'/cain_ssim_epoch{}.pkl'.format(str(epoch).zfill(2)), 'wb+') as f:
            pickle.dump(ssim_dict, f)
        # with open(output_dir+'/cain_vmaf_epoch{}.pkl'.format(str(epoch).zfill(2)), 'wb+') as f:
